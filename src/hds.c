...
int hbuf_init(hbuf_t* b, size_t s) {
  HC_RET_WHEN(!b, HC_RET(HC_RET_BUF_BASE, HC_RET_PARAM(0)));
  HC_RET_WHEN(HBUF_DATA_SIZE_MAX < s,
                 HC_RET(HC_RET_BUF_BASE, HC_RET_PARAM(2)));

  b->size = HBUF_DATA_SIZE_MIN;
  HC_GETMINPOW(s, b->size);
  b->len = 0;
  b->data = (uint8_t*)HC_ALLOC(b->size);
  return HC_RET_OK;
}
int hbuf_deinit(hbuf_t* b) {
  HC_RET_WHEN(!b, HC_RET(HC_RET_BUF_BASE, HC_RET_PARAM(0)));
  HC_RET_WHEN(!b->size, HC_RET(HC_RET_BUF_BASE, HC_RET_PARAM(1)));
  HC_RET_WHEN(!b->data, HC_RET(HC_RET_BUF_BASE, HC_RET_PARAM(2)));
...
static int hbuf_realloc(hbuf_t* b, size_t wish_size) {
  HC_RET_WHEN(wish_size <= b->size, HC_RET_OK);

  hbuf_t target = *b;
  b->size = HBUF_DATA_SIZE_MIN;
  HC_GETMINPOW(wish_size, target.size);
  HC_RET_WHEN(HBUF_DATA_SIZE_MAX < target.size,
                 HC_RET(HC_RET_BUF_BASE, HC_RET_NOSPACE));

  target.data = HC_REALLOC(target.data, target.size);
  HC_RET_WHEN(!target.data, HC_RET(HC_RET_BUF_BASE, HC_RET_MALLOC));
...
int hbuf_push(hbuf_t* b, const void* d, size_t dsize) {
  HC_RET_WHEN(!b, HC_RET(HC_RET_BUF_BASE, HC_RET_PARAM(0)));
  HC_RET_WHEN(!d, HC_RET(HC_RET_BUF_BASE, HC_RET_PARAM(1)));
  HC_RET_WHEN(!dsize, HC_RET(HC_RET_BUF_BASE, HC_RET_PARAM(2)));
  HC_RET_WHEN(!b->size, HC_RET(HC_RET_BUF_BASE, HC_RET_PARAM(3)));
...
int hrp_init(hrq_t* rq, uint32_t s) {
  HC_RET_WHEN(!rq, HC_RET(HC_RET_BASE_RQ, HC_RET_PARAM(0)));
  HC_RET_WHEN(!s, HC_RET(HC_RET_BASE_RQ, HC_RET_PARAM(1)));
  HC_RET_WHEN(HBUF_DATA_SIZE_MAX < s,
                 HC_RET(HC_RET_BASE_RQ, HC_RET_PARAM(2)));

  rq->size = HBUF_DATA_SIZE_MIN;
  HC_GETMINPOW(s, rq->size);
  rq->data = (uint8_t*)HC_ALLOC(rq->size);
  rq->in = 0;
  rq->out = 0;
  return HC_RET_OK;
}
int hrp_deinit(hrq_t* rq) {
  HC_RET_WHEN(!rq, HC_RET(HC_RET_BASE_RQ, HC_RET_PARAM(0)));
  HC_RET_WHEN(!rq->size, HC_RET(HC_RET_BASE_RQ, HC_RET_PARAM(1)));
  HC_RET_WHEN(!rq->data, HC_RET(HC_RET_BASE_RQ, HC_RET_PARAM(2)));
  HC_FREE(rq->data);
  rq->data = NULL;
...
int hrp_push(hrq_t* rq, void* data, uint32_t size) {
  HC_RET_WHEN(!rq, HC_RET(HC_RET_BASE_RQ, HC_RET_PARAM(0)));
  HC_RET_WHEN(!data, HC_RET(HC_RET_BASE_RQ, HC_RET_PARAM(1)));
  HC_RET_WHEN(!size, HC_RET(HC_RET_BASE_RQ, HC_RET_PARAM(2)));
  HC_RET_WHEN(!rq->size, HC_RET(HC_RET_BASE_RQ, HC_RET_PARAM(3)));
...
int hlist_clear(hlist_t* l) {
  HC_RET_WHEN(!l, HC_RET(HC_RET_BASE_LIST, HC_RET_PARAM(0)));
  l->head = NULL;
  l->size = 0;
  return HC_RET_OK;
}
...
int hlist_append(hlist_t* l, hlistitem_t* i) {
  HC_RET_WHEN(!l, HC_RET(HC_RET_BASE_LIST, HC_RET_PARAM(0)));
  HC_RET_WHEN(!i, HC_RET(HC_RET_BASE_LIST, HC_RET_PARAM(1)));
...
int hlist_insert(hlist_t* l, hlistitem_t* i, uint32_t index) {
  HC_RET_WHEN(!l, HC_RET(HC_RET_BASE_LIST, HC_RET_PARAM(0)));
  HC_RET_WHEN(!i, HC_RET(HC_RET_BASE_LIST, HC_RET_PARAM(1)));
  HC_RET_WHEN(index >= l->size, HC_RET(HC_RET_BASE_LIST, HC_RET_PARAM(2)));
...
hlistitem_t* hlist_at(hlist_t* l, uint32_t index) {
  HC_RET_WHEN(!l, NULL);
  HC_RET_WHEN(index >= l->size, NULL);
  HC_RET_WHEN(0 == index, l->head);
...
hlistitem_t* hlist_remove(hlist_t* l, uint32_t index) {
  HC_RET_WHEN(!l || index >= l->size, NULL);
...
hlistitem_t* hlist_find_first(hlist_t* l, hlistitem_t* tar,hlist_compare_t cmp) {
  HC_RET_WHEN(!l || !tar || !cmp || !l->size, NULL);
...
int hislist_foreach(hlist_t* l, hlist_foreach_t f) {
  HC_RET_WHEN(!l, HC_RET(HC_RET_BASE_LIST, HC_RET_PARAM(0)));
  HC_RET_WHEN(!f, HC_RET(HC_RET_BASE_LIST, HC_RET_PARAM(1)));
...
//     HC_RET_WHEN(!h,   HC_RET(HC_RET_BASE_HASH, HC_RET_PARAM(0)));
//     HC_RET_WHEN(!key, HC_RET(HC_RET_BASE_HASH, HC_RET_PARAM(1)));
...
//     if (!found) return HC_RET(HC_RET_BASE_HASH, HC_RET_NOTFOUND);
//     return HC_RET_OK;
// }
